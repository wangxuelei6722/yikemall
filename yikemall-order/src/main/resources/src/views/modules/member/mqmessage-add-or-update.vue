<template>
  <el-dialog
    ...
    <el-form-item label="0-新建1-已发送2错误抵达3-已抵达" prop="messageStatus">
      <el-input v-model="dataForm.messageStatus" placeholder="0-新建1-已发送2错误抵达3-已抵达"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          messageStatus: [
            { required: true, message: '0-新建1-已发送2错误抵达3-已抵达不能为空', trigger: 'blur' }
          ],
          createTime: [
            { required: true, message: '不能为空', trigger: 'blur' }
          ],
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.messageId) {
            this.$http({
              url: this.$http.adornUrl(`/member/mqmessage/info/${this.dataForm.messageId}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/member/mqmessage/${!this.dataForm.messageId ? 'save' : 'update'}`),
              method: 'post',
